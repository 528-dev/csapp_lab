...
0000000000400f43 <phase_3>:
  ...
  400f6a:	83 7c 24 08 07       	cmpl   $0x7,0x8(%rsp) # 判断第三个参数(输入的第一个数字)和 7 的大小
  400f6f:	77 3c                	ja     400fad <phase_3+0x6a> # 如果 输入的第一个数字 > 7 跳转到 bomb
  400f71:	8b 44 24 08          	mov    0x8(%rsp),%eax # 将输入的第一个数字传给 %eax
  400f75:	ff 24 c5 70 24 40 00 	jmpq   *0x402470(,%rax,8) # gdb x /8w 0x402470  查看 switch 表
  400f7c:	b8 cf 00 00 00       	mov    $0xcf,%eax # 输入的第一个数为 0 入口
  400f81:	eb 3b                	jmp    400fbe <phase_3+0x7b>
  400f83:	b8 c3 02 00 00       	mov    $0x2c3,%eax # 输入的第一个数为 2 入口
  400f88:	eb 34                	jmp    400fbe <phase_3+0x7b>
  400f8a:	b8 00 01 00 00       	mov    $0x100,%eax # 输入的第一个数为 3 入口
  400f8f:	eb 2d                	jmp    400fbe <phase_3+0x7b>
  400f91:	b8 85 01 00 00       	mov    $0x185,%eax # 输入的第一个数为 4 入口
  400f96:	eb 26                	jmp    400fbe <phase_3+0x7b>
  400f98:	b8 ce 00 00 00       	mov    $0xce,%eax # 输入的第一个数为 5 入口
  400f9d:	eb 1f                	jmp    400fbe <phase_3+0x7b>
  400f9f:	b8 aa 02 00 00       	mov    $0x2aa,%eax # 输入的第一个数为 6 入口
  400fa4:	eb 18                	jmp    400fbe <phase_3+0x7b>
  400fa6:	b8 47 01 00 00       	mov    $0x147,%eax # 输入的第一个数为 7 入口
...
000000000040100c <phase_4>:
  ...
  401056:	74 05                	je     40105d <phase_4+0x51> # 第二个输入为0 跳过bomb
...
0000000000401062 <phase_5>:
  ...
  40107f:	83 f8 06             	cmp    $0x6,%eax # 比较输入字符串长度和6
  ...
  4010a0:	88 54 04 10          	mov    %dl,0x10(%rsp,%rax,1) # 将找到的字符依次传给 %rsp[10+%rax]处
...
00000000004010f4 <phase_6>:
  ...
  401106:	e8 51 03 00 00       	callq  40145c <read_six_numbers> # ----------第一部分:保证六个数字为1,2,3,4,5,6的一种排序
  ...
  401151:	eb c1                	jmp    401114 <phase_6+0x20> #---------------------------------------------第一部分结束
  401153:	48 8d 74 24 18       	lea    0x18(%rsp),%rsi # 数组尾地址 + 4 -> %rsi ------------第二部分:将数组的每个数用7减去
  ...
  40116d:	75 f1                	jne    401160 <phase_6+0x6c> # 循环直到遍历完整个数组 -----------------------第二部分结束
  40116f:	be 00 00 00 00       	mov    $0x0,%esi # %rsi 一开始为0, 循环中一直在 +4 ------------第三部分:按数组的顺序排列链表
  401174:	eb 21                	jmp    401197 <phase_6+0xa3>
  401176:	48 8b 52 08          	mov    0x8(%rdx),%rdx # (%rdx) + 8 -> %rdx ---------------------]-------}
  40117a:	83 c0 01             	add    $0x1,%eax # %eax += 1
  40117d:	39 c8                	cmp    %ecx,%eax # 拿数组中的数和 从 0x1 开始不断增加的 %eax比较
  ...
  401191:	48 83 fe 18          	cmp    $0x18,%rsi # 不断循环直到所有的节点的指针都移到栈中                            
  401195:	74 14                	je     4011ab <phase_6+0xb7> #                          
  401197:	8b 0c 34             	mov    (%rsp,%rsi,1),%ecx # 将数组中的数依次给 %ecx       
  40119a:	83 f9 01             	cmp    $0x1,%ecx #                                      
  40119d:	7e e4                	jle    401183 <phase_6+0x8f> # 数组中的数 == 1 说明不需要向后遍历链表,跳转------(
  40119f:	b8 01 00 00 00       	mov    $0x1,%eax # 1 -> %eax
  4011a4:	ba d0 32 60 00       	mov    $0x6032d0,%edx # 0x6032d0 -> %edx
  4011a9:	eb cb                	jmp    401176 <phase_6+0x82> # -----------------------------------------{---第三部分结束
  4011ab:	48 8b 5c 24 20       	mov    0x20(%rsp),%rbx # 指针数组首值-----------第四部分开始:将链表的顺序修改为我们数组中的顺序
  ...
  4011d0:	eb eb                	jmp    4011bd <phase_6+0xc9> # --------------------------------------------第四部分结束
